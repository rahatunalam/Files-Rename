...
| Script | What it does |
|---|---|
| **Script 1 — Rename & Move** | Renames `.png` files to sequentially numbered `.jpg` filenames and moves them to a new folder |
| **Script 2 — Convert & Rename** | Opens `.jpg` images, converts them properly using Pillow, and saves them as sequentially numbered `.jpg` files in a new folder |
...
```python
folder = r'F:\download\IOT'           # Folder containing your .png files
...
```python
for i, file in enumerate(files, start=301):   # Change 301 to any number
```

### Usage

```bash
python rename_move.py
```
...
```python
source_folder = r'G:\Exam'             # Folder containing your source .jpg files
...
```python
for i, file in enumerate(png_files, start=1):  # Change 1 to any number
```

### Usage

```bash
python convert_rename.py
```
